...
def create_content_writer(model):
...
agent = create_content_writer(model)
...
#### Start a server
#### Use AI Agent Marketplace Live Runtime to Bring Agent Live
    ...
    try:
        with Live(display.spinner, console=console, refresh_per_second=10, transient=True) as live:
            ...
            async for chunk in agent.astream(
                ...
                config={"configurable": {"thread_id": "live-runtime"}},
                stream_mode="values",
            ):
